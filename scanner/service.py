...
from features.broker_gateway import dhan_quote_post_blocking as _dhan_quote_post_blocking
...
def _build_scanner_daily_dhan_stock_instruments(db) -> tuple[list[int], dict[str, dict[str, Any]]]:
    """Dhan equivalent of _build_scanner_daily_stock_instruments — keys the meta
    map by "NSE_EQ:{security_id}" instead of "EXCHANGE:TRADINGSYMBOL" since
    Dhan's quote API addresses instruments by numeric security_id, not symbol."""
    rows = list(
        db[STOCKS_COLLECTION].find(
            {},
            {
                "_id": 0, "symbol": 1, "tradingsymbol": 1, "series": 1,
                "kite_token": 1, "token": 1, "instrument_token": 1,
                "dhan_security_id": 1,
            },
        )
    )
    ...
    for row in rows:
        if str(row.get("series") or "").strip().lower() == "index":
            continue
        raw_id = str(row.get("dhan_security_id") or "").strip()
        try:
            security_id = int(float(raw_id)) if raw_id else 0
        except (ValueError, TypeError):
            security_id = 0
        if not security_id:
            skipped_no_id += 1
        ...
        meta_map[f"NSE_EQ:{security_id}"] = row

    print(
        f"📘 Dhan stock loader: total_rows={len(rows)} loaded={len(security_ids)} skipped_no_id={skipped_no_id}",
        flush=True,
    )
    return security_ids, meta_map


def _build_scanner_daily_dhan_index_instruments(db) -> tuple[dict[str, list[int]], dict[str, dict[str, Any]]]:
    """Dhan equivalent of _build_scanner_daily_index_instruments. Returns
    ({segment: [security_id, ...]}, {"segment:security_id": meta}) — GOLDBEES is
    an NSE-listed ETF (segment NSE_EQ), every other index is IDX_I, same split
    sync_scanner_historical_data already uses for Dhan indices."""
    index_rows = _load_scanner_index_master(db, {})
    ids_by_segment: dict[str, list[int]] = {}
    meta_map: dict[str, dict[str, Any]] = {}
    skipped_no_id = 0

    for kite_key, index_row in index_rows.items():
        if kite_key == "GOLDBEES":
            segment, security_id = "NSE_EQ", int(DHAN_GOLDBEES_SECURITY_ID)
        else:
            segment = "IDX_I"
            security_id = int(DHAN_INDEX_SECURITY_IDS.get(kite_key, 0) or 0)
        if not security_id:
            skipped_no_id += 1
            continue
        ids_by_segment.setdefault(segment, []).append(security_id)
        meta_map[f"{segment}:{security_id}"] = {
            "normalized_symbol": index_row["normalized_symbol"],
            "raw_symbol": index_row["raw_symbol"],
        }

    print(
        f"📗 Dhan index loader: loaded={sum(len(v) for v in ids_by_segment.values())} skipped_no_id={skipped_no_id}",
        flush=True,
    )
    return ids_by_segment, meta_map


def _fetch_dhan_quotes_in_batches(
    client_id: str,
    access_token: str,
    ids_by_segment: dict[str, list[int]],
    *,
    ...
    """Bulk-fetch today's OHLC for every id in ids_by_segment via Dhan's
    /v2/marketfeed/quote — one call covers up to ~500 ids per segment (same cap
    broker_gateway.py's own quote batching uses), so the whole stocks_list +
    index master fits in a handful of requests on the shared ~1 req/sec Dhan
    rate gate (wait_for_dhan_slot, via dhan_quote_post_blocking) instead of one
    request per symbol like sync_scanner_historical_data's chart-endpoint loop.
    Each returned payload is normalized to carry "instrument_token" so it can
    feed straight into _transform_scanner_daily_stock_quote /
    _transform_scanner_daily_index_quote unchanged — Dhan's quote packet
    already uses the same ohlc/last_price/volume shape Kite's quote() does.
    A security_id Dhan doesn't recognize (stale/wrong DB data) is simply
    absent from the response — no error, no retry burned on it, unlike the
    per-symbol historical endpoint's DH-905 retry loop.
    """
    quotes: dict[str, dict[str, Any]] = {}
    if not ids_by_segment:
        return quotes, 0
    max_len = max((len(ids) for ids in ids_by_segment.values()), default=0)
    num_batches = (max_len + batch_size - 1) // batch_size if max_len else 0

    for batch_idx in range(num_batches):
        req_body = {
            segment: ids[batch_idx * batch_size:(batch_idx + 1) * batch_size]
            for segment, ids in ids_by_segment.items()
            if ids[batch_idx * batch_size:(batch_idx + 1) * batch_size]
        }
        if not req_body:
            continue
        print(f"📦 Dhan quote batch {batch_idx + 1}/{num_batches} segments={list(req_body.keys())}", flush=True)
        resp = _dhan_quote_post_blocking(req_body, access_token, client_id, timeout=15.0)
        if resp is None or resp.status_code != 200:
            print(f"⚠️ Dhan quote batch {batch_idx + 1} failed status={getattr(resp, 'status_code', None)}", flush=True)
            continue
        raw = resp.json()
        data = raw.get("data") or raw
        for segment in req_body.keys():
            for security_id, payload in (data.get(segment) or {}).items():
                if not isinstance(payload, dict):
                    continue
                payload = dict(payload)
                payload["instrument_token"] = str(security_id)
                quotes[f"{segment}:{security_id}"] = payload

    return quotes, num_batches
...
def sync_scanner_daily_market_snapshot() -> dict[str, Any]:
    ...
    if broker == "dhan":
        ...
        if not access_token or not client_id:
            raise ValueError("Active Dhan credentials not configured.")
        print("✅ Dhan client ready.", flush=True)

        stock_ids, stock_meta_map = _build_scanner_daily_dhan_stock_instruments(db)
        index_ids_by_segment, index_meta_map = _build_scanner_daily_dhan_index_instruments(db)
        print(f"✅ Loaded {len(stock_ids)} stock instruments from {STOCKS_COLLECTION}", flush=True)
        print(
            f"✅ Loaded {sum(len(v) for v in index_ids_by_segment.values())} index instruments from {INDEX_STOCKS_COLLECTION}",
            flush=True,
        )

        stock_quotes, stock_batches = _fetch_dhan_quotes_in_batches(client_id, access_token, {"NSE_EQ": stock_ids})
        index_quotes, index_batches = _fetch_dhan_quotes_in_batches(client_id, access_token, index_ids_by_segment)
        stock_missing = [key for key in stock_meta_map if key not in stock_quotes]
        index_missing = [key for key in index_meta_map if key not in index_quotes]
        # Both branches converge on these two names below (return dict, missing-quote
        # logging) — Kite's loader returns them directly, Dhan's meta maps double as
        # the instrument list here so the rest of the function doesn't need to care
        # which broker produced the data.
        stock_instruments = list(stock_meta_map.keys())
        index_instruments = list(index_meta_map.keys())
    else:
        kite = _build_kite_client_from_config()
...
def sync_scanner_historical_data(
    ...
    try:
        ...
        for _, index_row in sorted(index_rows.items()):
            ...
            if broker == "dhan":
                is_dhan_index = kite_key != "GOLDBEES"
                if not is_dhan_index:
                ...
                # Dhan's /v2/charts/historical (EOD) rejects every IDX_I request with
                # DH-905 regardless of params/date-range — it doesn't support index
                # instruments (see fetch_dhan_daily_index_candles_cached's docstring,
                # already used for this same reason by the chart-feed path below).
                # Daily index bars must instead be derived from /v2/charts/intraday,
                # which caps at 90 days per call instead of the EOD endpoint's 365.
                chunk_days = DHAN_INTRADAY_CHUNK_DAYS if is_dhan_index else DHAN_HISTORICAL_CHUNK_DAYS
                while current_from <= to_dt:
                    current_to = min(current_from + timedelta(days=chunk_days), to_dt)
                    try:
                        if is_dhan_index:
                            all_candles.extend(_fetch_dhan_daily_index_candles_cached(
                                dhan_security_id, dhan_access_token, normalized_symbol, current_from, current_to
                            ))
                        else:
...
def _load_dhan_commodity_master() -> dict[str, list[dict]]:
    try:
        from api import _get_dhan_commodity_master as _impl
    except Exception:
        from backend.api import _get_dhan_commodity_master as _impl  # type: ignore
    return _impl()
