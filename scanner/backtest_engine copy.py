    ...
    for suffix, days in periods.items():
        ...
        df[f"vol_{suffix}"] = (
            df.groupby("h_symbol", group_keys=False)["ret"]
              .transform(lambda x: x.rolling(days, min_periods=5).std() * 100)
        )
...
# Sigma Score Core
# ===============================
def compute_sigma_scores_core(df_stocks, ev_date, expr, df_meta=None, mode="backtest"):
    year = ev_date.year
    ...
    for name, days in vol_periods.items():
        df_hist[name] = df_hist.groupby("h_symbol")["ret"].transform(
            lambda x: x.rolling(days, min_periods=5).std() * 100
        )
...
def ultra_backtest_v2_fastest_fixed_v3(
    ...
    EXECUTION_TIMING="next_trading_day",  # "same_day" or "next_trading_day"
    ...
    def sigma_score_snapshot(ev_date: pd.Timestamp):
        ...
        for sym, (idx, prices, opens, highs, lows) in _price_cache.items():
            ...
            def rstd(d):
                ...
                return float(np.std(w, ddof=1)) * 100 if len(w) >= 5 else 0.0
    ...
    holdings, trades, equity, snapshots = {}, [], [], []
    first_trading_day = df_index.index.min()

    # Determine initial regime state
    if EXIT_REGIME_FILTER_STATUS:
        past = [d for d in sorted(sig_map.keys()) if d <= start_date]
        state = sig_map[past[-1]] if past else "Buy"
    else:
        state = "Buy"
    last_signal = state
    ...
    # Initial positions
    if state == "Buy":
        snap0 = sigma_score_snapshot(first_trading_day)
        if not snap0.empty and top_n > 0:
            top_syms = snap0.head(top_n).index.tolist()
            alloc = cash / len(top_syms)
            for sym in top_syms:
                p = snap0.loc[sym, "ch_closing_price"]
                if p and p > 0:
                    qty = int(alloc // p)
                    if qty > 0:
                        holdings[sym] = qty
                        rank = int(snap0.loc[sym, "rank"]) if "rank" in snap0.columns else None
                        add_trade_row(first_trading_day, sym, "BUY_INITIAL", qty, float(p), rank)
    else:
        # Supertrend says SELL → go into GoldBees
        if UNCORRELATED_ASSET_STATUS:
            gpx = get_latest_index_price(df_gold, first_trading_day)
            if gpx and gpx > 0:
                qty = int((cash * gold_alloc) // gpx)
                if qty > 0:
                    holdings["gold_bees"] = qty
                    add_trade_row(first_trading_day, "gold_bees", "BUY_GOLDBEES_INIT", qty, float(gpx))

    # Record initial snapshot
    equity.append({
        "date": first_trading_day,
        "value": float(round(portfolio_value_on(first_trading_day), 2)),
        "cash": float(round(cash, 2)),
        "holdings_count": int(len(holdings)),
        "state": state
    })
    push_snapshot(first_trading_day)
    # ===================================================
    ...
    # --- Normalize all timestamps to date-only (drop tz + time) ---
    def normalize_date(ts):
        if pd.isna(ts):
            return None
        if not isinstance(ts, pd.Timestamp):
            ts = pd.Timestamp(ts)
        return ts.normalize()  # strips time component and timezone
    ...
    for ev_date in daily_dates: #event_dates #daily_dates
        ...
        if EXIT_REGIME_FILTER_STATUS and ev_date in sig_map and sig_map[ev_date] != last_signal:
            ...
            if sig == "Sell":
                for sym, qty in list(holdings.items()):
                    if sym == "gold_bees":  # don’t sell gold
                        continue
                    px = fast_price(sym, trade_day(ev_date, False))
                    if not px:
                        continue
                    if FILTER_ACTION == "half_portfolio":
                        sell_qty = qty // 2
                        if sell_qty > 0:
                            add_trade_row(trade_day(ev_date, False), sym, "SELL_HALF_SIGNAL", sell_qty, px)
                            remain = qty - sell_qty
                            if remain > 0:
                                holdings[sym] = remain
                            else:
                        ...
                        add_trade_row(trade_day(ev_date, False), sym, "SELL_SIGNAL", qty, px, rank)
                        holdings.pop(sym, None)
                ...
                if not snap.empty and top_n > 0:
                    ...
                    if candidates and cash > 0:
                        ...
                        for sym in candidates:
                            ...
                            # p = snap.loc[sym, "ch_closing_price"]
                            if p and p > 0:
                                qty = int(alloc // p)
                                if qty > 0:
                                    holdings[sym] = qty
        ...
        if (ev_date in effective_rebalance_dates) and state == "Buy" and ("gold_bees" not in holdings):
            ...
            # -------- Phase 2: BUY_REBALANCE --------
            buy_d = trade_day(ev_date, True)
            current_count = len(holdings)
            if(BUY_RENBALANCE_SCORE_SNAPSHOT_STATUS == True):
            ...
            if remaining_slots > 0:
                ...
                if candidates and cash > 0:
                    ...
                    buy_block = []
                    for sym in candidates:
                        # print("symsym",sym, buy_d, "sell_d",sell_d)
                        p = fast_price(sym, buy_d)
                        # p = snap.loc[sym, "ch_closing_price"]
                        # print("ppp",p, "pxpx",px)
                        if p and p > 0:
        ...
        if not df_eq.empty:
            ...
            if not closed_df.empty:
                goldbees_realized_map = (
                    closed_df[closed_df["symbol"].str.lower() == "gold_bees"]
                    .groupby("month")["roi"].sum()
                    .to_dict()
                )
            else:
                goldbees_realized_map = {}
    ...
    return metrics, df_eq, df_tr, monthly_json, monthly_stats_json, equity_curve_json, snapshots_df, monthly_equity_json, yearly_equity_json, daily_json, weekly_json, monthly_final_json, yearly_final_json, quarterly_json, monthly_closed_json, closed_df
